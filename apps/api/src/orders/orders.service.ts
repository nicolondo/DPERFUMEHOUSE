...
export class OrdersService {
  ...
  async createOrder(data: CreateOrderBodyDto, sellerId: string) {
    ...
    // Promo discount logic — limit is per-seller globally
    let promoDiscount = 0;
    let promoPercent = 0;
    if (data.applyPromoDiscount) {
      ...
      const effectivePercent = parseFloat(settingPercent || '0');
      const effectiveLimit = parseInt(settingLimit || '0');
  ...
  async syncOdoo(orderId: string, existingOdooSoId?: number) {
    ...
    if (
      ...
      throw new BadRequestException(
        `Order must be in PAID, SHIPPED or DELIVERED status to sync with Odoo`,
      );
    }
