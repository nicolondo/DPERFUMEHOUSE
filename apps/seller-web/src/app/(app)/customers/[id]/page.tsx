    ...
    <div className="pb-24">
      ...
      <div className="px-4 space-y-4">
        ...
        <button
          onClick={async () => {
            ...
            try {
              ...
              if (url && leadId) {
                setQuestionnaireData({ leadId, url });
        ...
        >
          ...
          {createLead.isPending ? 'Generando...' : 'Enviar Cuestionario de Fragancias'}
        </button>

        {/* Questionnaire Send Method Modal */}
        <Modal
          isOpen={showQuestionnaireModal}
          onClose={() => { setShowQuestionnaireModal(false); setQuestionnaireData(null); }}
          title="Enviar cuestionario"
        >
          <p className="text-sm text-white/60 mb-5">¿Cómo quieres enviar el cuestionario a {customer.name.split(' ')[0]}?</p>
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-white/40">{customer.phone ? formatPhone(customer.phone) : 'Sin teléfono registrado'}</p>
              </div>
            </button>
            <button
              onClick={async () => {
                if (!questionnaireData) return;
                try {
                  await sendEmail.mutateAsync(questionnaireData.leadId);
                  setShowQuestionnaireModal(false);
                  setQuestionnaireData(null);
                } catch {
                  // handled by react-query
              ...
              disabled={!customer.email || sendEmail.isPending}
            ...
            >
              ...
              <div>
                <p className="text-sm font-medium text-white">{sendEmail.isPending ? 'Enviando...' : 'Correo electrónico'}</p>
